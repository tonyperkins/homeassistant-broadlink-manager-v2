...
def learn_command():
    ...
    try:
        ...
        save_destination = data.get("save_destination", "manager_only")  # manager_only, integration_only, both
        ...
        if result.get("success"):
            ...
            if device_id:
                ...
                if save_destination == "manager_only":
                    logger.info(f"ℹ️ Deletion of '{command}' from integration storage will happen after code is fetched")

                # Log if integration_only (no devices.json update)
                if save_destination == "integration_only":
                    logger.info(f"ℹ️ Skipping devices.json update for '{command}' - save_destination=integration_only")
...
def test_command():
    ...
    try:
        ...
        if is_smartir:
            ...
            if not device_code:
                return (
                    jsonify({"success": False, "error": "SmartIR device_code not found"}),
                    400,
                )
            ...
            if command in ["pending", "error"]:
                ...
                logger.info(f"Sending Broadlink raw code to HA (code length: {len(command)} chars)")
...
def delete_command_from_storage():
    ...
    try:
        ...
        logger.info(f"🗑️ Deleting command '{command}' from Broadlink storage for device '{device}'")
        ...
        if device_manager:
            ...
            for dev_id, dev_data in all_devices.items():
                ...
                if storage_key == device:
                    ...
                    logger.info(f"🔍 Found entity_id '{entity_id}' for storage key '{device}' (device_id: {dev_id})")
                    break
        ...
        result = loop.run_until_complete(
            web_server._make_ha_request("POST", "services/remote/delete_command", service_payload)
        )
        loop.close()
        ...
        if result is not None:
            logger.info(f"✅ Command '{command}' deleted from Broadlink storage via {entity_id}")
...
def get_untracked_commands():
    ...
    try:
        ...
        for device_id, device_data in all_devices.items():
            ...
            logger.debug(f"  Tracked commands for {device_name}: {tracked_commands[device_name]}")
        ...
        for device_name, commands in broadlink_commands.items():
            ...
            if untracked_for_device:
                logger.info(f"  ⚠️ Untracked commands: {list(untracked_for_device.keys())}")
            else:
...
def import_commands():
    ...
    try:
        ...
        logger.info(f"📥 Parsed: device_id={device_id}, source_device={source_device}, commands={commands}")

        if not device_id or not source_device or not commands:
            error_msg = (
                f"Missing required fields: device_id={device_id}, source_device={source_device}, commands={commands}"
            )
            logger.error(f"❌ {error_msg}")
            return (
                jsonify(
        ...
        logger.info(f"📦 Fetching commands from Broadlink storage for device '{source_device}'...")
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        broadlink_commands = loop.run_until_complete(web_server._get_all_broadlink_commands())
        loop.close()
        ...
        logger.info(f"📦 Found {len(storage_commands)} commands in storage for '{source_device}'")
        ...
        logger.info(f"✅ Imported {imported_count} commands from '{source_device}' to device '{device_id}'")
        ...
        for device_id, device_data in managed_devices.items():
            ...
            for cmd_name, cmd_data in storage_commands.items():
                ...
                if cmd_name in recently_deleted:
                    logger.debug(f"  ⏭️ Skipping recently deleted command '{cmd_name}' during sync")
                    continue
                ...
                if cmd_name not in tracked_commands:
                    ...
                    if isinstance(existing_cmd, dict):
                        existing_type = existing_cmd.get("command_type") or existing_cmd.get("type")
                    else:
                        existing_type = None
            ...
            commands_to_remove = [cmd_name for cmd_name in tracked_commands.keys() if cmd_name not in storage_commands]
...
def learn_command_direct_stream():
    ...
    def generate():
        try:
            ...
            device_manager = DeviceManager(storage_path=str(web_server.broadlink_manager_path))
...
def learn_command_direct():
    ...
    try:
        ...
        if not learner.authenticate():
            return (
                jsonify({"success": False, "error": "Failed to authenticate with device"}),
                500,
            )
        ...
        if not success:
            return (
                jsonify({"success": False, "error": "Failed to save command to storage"}),
                500,
            )
...
def test_command_direct():
    ...
    try:
        ...
        if not command_data:
            return (
                jsonify({"success": False, "error": f"Command {command_name} not found"}),
                404,
            )
