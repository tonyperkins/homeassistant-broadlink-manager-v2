    ...
    try:
        ...
        if device_type == "smartir":
            ...
            if "controller_device" in data or "controller_device" in smartir_config:
                entity_data["controller_device"] = data.get("controller_device") or smartir_config.get(
                    "controller_device", ""
                )
    ...
    try:
        ...
        for device_name, commands in broadlink_commands.items():
            if device_name not in tracked_device_names:
                ...
                remaining_commands = [
                    cmd for cmd in commands.keys() if not web_server._is_recently_deleted(device_name, cmd)
                ]
                ...
                if remaining_commands:
                    ...
                    logger.info(f"Device '{device_name}' excluded: all {len(commands)} commands recently deleted")
...
def delete_untracked_device(device_name):
    ...
    try:
        ...
        if not broadlink_entity:
            loop.close()
            return (
                jsonify({"error": f"Could not determine Broadlink entity for device {device_name}"}),
                400,
            )
...
def create_managed_device():
    ...
    try:
        ...
        if device_type == "smartir":
            ...
            if not broadlink_entity:
                return (
                    jsonify({"error": "broadlink_entity is required for Broadlink devices"}),
                    400,
                )
