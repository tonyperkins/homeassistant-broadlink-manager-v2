    ...
    try:
        ...
        if "broadlink_entity" in data:
            ...
            if old_entity and new_entity and old_entity != new_entity:
                ...
                logger.info(
                    f"Blaster changed for '{device_id}': {old_entity} -> {new_entity}, cleared stale connection info"
                )
        if "enabled" in data:
        ...
        if device_type == "smartir":
            ...
            if "controller_device" in data or "controller_device" in smartir_config:
                ...
                new_controller = data.get("controller_device") or smartir_config.get(
                    "controller_device", ""
                )
                ...
                if (
                    old_controller
                    and new_controller
                    and old_controller != new_controller
                ):
                    entity_data.pop("connection", None)
                    logger.info(
                        f"Controller changed for SmartIR device '{device_id}': "
                        f"{old_controller} -> {new_controller}, "
                        "cleared stale connection info"
                    )
...
def update_managed_device(device_id):
    ...
    try:
        ...
        if old_broadlink and new_broadlink and old_broadlink != new_broadlink:
            ...
            logger.info(
                f"Blaster changed for managed device '{device_id}': "
                f"{old_broadlink} -> {new_broadlink}, cleared stale connection info"
            )
        ...
        if "smartir_config" in data and existing_device.get("device_type") == "smartir":
            ...
            if "controller_device" in smartir_config:
                ...
                if (
                    ...
                    updated_data.pop("connection", None)
                    logger.info(
                        f"Controller changed for SmartIR managed device "
                        f"'{device_id}': {old_controller} -> "
                        f"{new_controller}, cleared stale connection info"
                    )
...
def delete_managed_device(device_id):
    ...
    try:
        ...
        # Check options from query parameters (more RESTful for DELETE)
        delete_commands = request.args.get("delete_commands", "false").lower() == "true"
        regenerate_entities = (
            request.args.get("regenerate_entities", "false").lower() == "true"
        )

        logger.info(
            f"Delete device options: delete_commands={delete_commands}, regenerate_entities={regenerate_entities}"
        )
        ...
        # If requested, regenerate entities to remove orphaned entity entries
        entities_regenerated = False
        if regenerate_entities and web_server:
            try:
                logger.info(
                    f"🔄 Regenerating entities after deleting device '{device_id}'..."
                )
                ...
                if broadlink_devices:
                    ...
                    # No devices left, create empty files
                    from pathlib import Path

                    config_path = Path(web_server.config_loader.get_config_path())
                    broadlink_manager_dir = config_path / "broadlink_manager"
                    broadlink_manager_dir.mkdir(parents=True, exist_ok=True)

                    package_file = broadlink_manager_dir / "package.yaml"
                    helpers_file = broadlink_manager_dir / "helpers.yaml"

                    for file_path in [package_file, helpers_file]:
                        with open(file_path, "w") as f:
                            f.write("# Auto-generated by Broadlink Manager\n")
                            f.write("# No devices configured\n")
                    entities_regenerated = True
                    logger.info("✅ Created empty entity files (no devices remaining)")
