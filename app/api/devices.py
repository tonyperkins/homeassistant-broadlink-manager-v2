...
def sync_device_area(device_id):
    ...
    try:
        ...
        if "." in device_id:
            ...
            sanitized_id = re.sub(r'_+', '_', sanitized_id)  # Remove duplicate underscores
            sanitized_id = sanitized_id.strip('_')  # Remove leading/trailing underscores
            
            entity_type = device_data.get("entity_type", "switch")
            full_entity_id = f"{entity_type}.{sanitized_id}"
            logger.info(f"Built entity_id: {full_entity_id} (from device_id: {device_id})")
