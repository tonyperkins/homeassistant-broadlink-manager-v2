...
class MigrationManager:
    ...
    async def check_and_migrate(self, broadlink_devices: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if existing_entities:
                logger.info(
                    f"📋 Existing Broadlink Manager user detected: {len(existing_entities)} entities already configured"
                )
                ...
                try:
                    ...
                    for device_name, commands in data.get("data", {}).items():
                        if isinstance(commands, dict) and commands:
                            ...
                            logger.info(
                                f"Discovered device '{device_name}' with {len(commands)} commands "
                                f"(Broadlink: {broadlink_entity}, Area: {area_name})"
                            )
    ...
    def _create_device_map(
        self, storage_files: List[Path], broadlink_devices: List[Dict[str, Any]]
