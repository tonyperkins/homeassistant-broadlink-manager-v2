"""Manages heartbeats for Broadlink devices to keep them online."""
...
class BroadlinkHeartbeat:
    ...
    async def async_unload(self) -> None:
        """Unload the heartbeat timer."""
        if self._unsubscribe is not None:
            self._unsubscribe()
            self._unsubscribe = None

    async def async_heartbeat(self, _: dt.datetime) -> None:
        """Send a heartbeat to all configured devices."""
        hass = self._hass
    ...
    def heartbeat(hosts: set[str]) -> None:
        """Send a ping to each device to feed its watchdog timer."""
        for host in hosts:
            try:
                blk.ping(host)
