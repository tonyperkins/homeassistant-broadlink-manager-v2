...
async def async_setup_entry(
    ...
    """Set up the Broadlink light from a config entry."""
...
class BroadlinkLight(BroadlinkEntity, LightEntity):
    ...
    def _update_state(self, data):
        """Update the state of the light entity.
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        """Turn on the light with specified attributes."""
        state = {"pwr": 1}
    ...
    async def _async_set_state(self, state):
        """Send a command to set the state of the light.

        Args:
            state: A dictionary representing the desired state.
        """
        device = self._device
