...
async def async_setup_platform(
    ...
    """Set up custom Broadlink switches from configuration.yaml.

    This method is for backward compatibility to support legacy YAML configurations.
    It imports the device and sets up custom IR/RF switches.
    Modern configurations should use the config flow.
    """
...
async def async_setup_entry(
    ...
    """Set up the Broadlink switch from a config entry."""
...
class BroadlinkSwitch(BroadlinkEntity, SwitchEntity, RestoreEntity, ABC):
    """Abstract base class for a Broadlink switch."""
    ...
    def __init__(self, device, command_on, command_off):
        ...
        Args:
            ...
            command_on: The command to turn the switch on.
            command_off: The command to turn the switch off.
        """
    ...
    async def async_added_to_hass(self) -> None:
        """Call when the switch is added to hass to restore its state."""
...
class BroadlinkRMSwitch(BroadlinkSwitch):
    """Representation of a custom IR/RF switch controlled by a Broadlink RM device."""

    def __init__(self, device, config):
        """Initialize the custom RM switch.

        Args:
            device: The Broadlink RM device object.
            config: The switch configuration.
        """
        super().__init__(
...
class BroadlinkSP2Switch(BroadlinkSP1Switch):
    """Representation of a Broadlink SP2 switch (and compatible models)."""
...
class BroadlinkMP1Slot(BroadlinkSwitch):
    """Representation of a single slot on a Broadlink MP1 power strip."""

    _attr_assumed_state = False
    _attr_has_entity_name = True

    def __init__(self, device, slot):
        """Initialize the MP1 slot switch.
    ...
    async def _async_send_packet(self, packet):
        """Send a packet to the device."""
        device = self._device
        # Send a packet to the device to turn the slot on or off
        try:
...
class BroadlinkBG1Slot(BroadlinkSwitch):
    """Representation of a single slot on a Broadlink BG1 smart outlet."""

    _attr_assumed_state = False
    _attr_has_entity_name = True

    def __init__(self, device, slot):
        """Initialize the BG1 slot switch.

        Args:
            device: The Broadlink device object.
            slot: The slot number (1-2).
        """
        super().__init__(device, 1, 0)
        self._slot = slot
