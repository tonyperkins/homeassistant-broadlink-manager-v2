"""Manages fetching data from Broadlink devices using DataUpdateCoordinator."""
...
def get_update_manager(device: BroadlinkDevice[_ApiT]) -> BroadlinkUpdateManager[_ApiT]:
    ...
    This factory function selects the appropriate update manager class based on the
    device type and instantiates it.

    Args:
        device: The Broadlink device object.

    Returns:
        An instance of a BroadlinkUpdateManager subclass.
    """
...
class BroadlinkUpdateManager(ABC, Generic[_ApiT]):
    """Abstract base class for a Broadlink update manager.

    This class uses DataUpdateCoordinator to periodically fetch data from a
    Broadlink device, manage its availability status, and handle communication errors.
    """
    ...
    async def async_update(self) -> dict[str, Any] | None:
        """Fetch data from the device and update its availability status."""
        try:
    ...
    async def async_fetch_data(self) -> dict[str, Any] | None:
        ...
        This method must be implemented by subclasses to retrieve the current
        state of the device.
        """


class BroadlinkA1UpdateManager(BroadlinkUpdateManager[blk.a1]):
    """Manages updates for Broadlink A1 environmental sensors."""
...
class BroadlinkA2UpdateManager(BroadlinkUpdateManager[blk.a2]):
    """Manages updates for Broadlink A2 environmental sensors."""
...
class BroadlinkMP1UpdateManager(BroadlinkUpdateManager[blk.mp1]):
    """Manages updates for Broadlink MP1 power strips."""
...
class BroadlinkMP1SUpdateManager(BroadlinkUpdateManager[blk.mp1s]):
    """Manages updates for Broadlink MP1S power strips with power monitoring."""
...
class BroadlinkRMUpdateManager(BroadlinkUpdateManager[blk.rm]):
    """Manages updates for Broadlink RM universal remotes."""
...
class BroadlinkSP1UpdateManager(BroadlinkUpdateManager[blk.sp1]):
    """Manages updates for Broadlink SP1 smart plugs."""
...
class BroadlinkSP2UpdateManager(BroadlinkUpdateManager[blk.sp2]):
    """Manages updates for Broadlink SP2 smart plugs (and compatible models)."""
...
class BroadlinkBG1UpdateManager(BroadlinkUpdateManager[blk.bg1]):
    """Manages updates for Broadlink BG1 smart outlets."""
...
class BroadlinkSP4UpdateManager(BroadlinkUpdateManager[blk.sp4]):
    """Manages updates for Broadlink SP4 smart plugs."""
...
class BroadlinkLB1UpdateManager(BroadlinkUpdateManager[blk.lb1]):
    """Manages updates for Broadlink LB1 smart bulbs."""
...
class BroadlinkThermostatUpdateManager(BroadlinkUpdateManager[blk.hysen]):
    """Manages updates for Hysen thermostats with Broadlink DNA."""
